...
public class CartService {
    ...
    public ResponseCartDTO addItemsToCart(RequestCartItemDTO requestCartItemDTO) {
        ...
        try {
            ...
            String username = requestCartItemDTO.getUsername();
            if(username == null){
                username = "unkwon";
            }
            return new ResponseCartDTO(Constants.SUCCESS_CODE, "0", Constants.SUCCESS_MESSAGE, Constants.SUCCESS_MESSAGE, savedCart.getToken(), username, cartDTO);
    ...
    public ResponseCartDTO getItemsFromCartByIds(RequestGetCartByIds requestGetCartByIds) {
        String token = requestGetCartByIds.getToken();
        List<Long> cartIds = requestGetCartByIds.getCartIds();
        ...
        try {
            ...
            return new ResponseCartDTO(Constants.SUCCESS_CODE, "0", Constants.SUCCESS_MESSAGE, "Cart retrieved successfully", token, cartDTOs);
        } catch (ResourceNotFoundException e) {
            return new ResponseCartDTO(Constants.ERROR_CODE, "1", "Failure", e.getMessage(), token);
        } catch (Exception e) {
            return new ResponseCartDTO(Constants.ERROR_CODE, "1", "Failure", "An unexpected error occurred: " + e.getMessage(), token
