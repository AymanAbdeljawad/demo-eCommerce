...
const Cart = () => {
  ...
  const handleCreateOrder = async () => {
    ...
    const sesLoginToken = Cookies.get('sesLoginToken');
    const orderData = {
      clientId: "0",
      tracingId: "0",
      errorCode: "a",
    ...
    try {
      const response = await axios.post('api/cart/addItemsToCart', orderData);
      if (response.data.clientId === 0) {
        ...
        const response1 = await axios.post('/api/payment/charge', orderData1);
